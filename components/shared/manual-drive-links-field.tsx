...
import { ExternalLink, Link2, Plus, X } from "@/lib/icons";
import { cn } from "@/lib/utils";

interface ManualDriveLinksFieldProps {
  className?: string;
  links: string[];
...
export function ManualDriveLinksField({
  className,
  links,
  onChange,
}: ManualDriveLinksFieldProps) {
    ...
    <div className={cn("space-y-3", className)}>
      <div className="rounded-xl border border-border bg-muted/30 p-3">
        <p className="text-xs leading-5 text-muted-foreground">
          ...
          lalu tempel link share file-nya di bawah.
        </p>
        <a
